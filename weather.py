...
def report_daily(data, date):
    ...
    for key in data:
        if date == key[0:8]:
            ...
            r = data[key]['r']
            round(r,2)
            display = display + f'{m:22}{tm:8}{t:13}{h:10}{r:10}\n'
    return display
...
def report_historical(data):
    ...
    for key in data:
        if h == key[0:8]:
            ...
            min_temp = min_temperature(data=data, date=h)
            max_temp = max_temperature(data=data, date=h)
            min_hum = min_humidity(data=data, date=h)
            max_hum = max_humidity(data=data, date=h)
            rain = tot_rain(data=data, date=h)
